...
const FIELDS = [
    ...
    { key: 'og_title',        label: 'OG Title',         hint: 'The bold title shown in WhatsApp/Facebook link previews.',                                       type: 'text'     },
    { key: 'og_description',  label: 'OG Description',   hint: 'The subtitle shown in link previews. Keep it under 160 characters.',                             type: 'textarea' },
    { key: 'og_image_url',    label: 'OG Image URL',     hint: 'The image shown when the homepage link is shared. Recommended size: 1200×630px.',                type: 'url'      },
    { key: 'og_url',          label: 'Site URL',         hint: 'The canonical URL of the site (e.g. https://mtechtestimonials.co.za).',                          type: 'url'      },
]
...
export default function Settings() {
    ...
    async function handleSave() {
        ...
        if (error) { setError('Failed to save settings: ' + error.message) }
        ...
        <div className="admin-page-content">
            <div className="edit-header">
                ...
                <button className={`btn-save ${saved ? 'saved' : ''}`} onClick={handleSave} disabled={saving}>
                    {saved ? <><Check size={15} /> Saved!</> : <><Save size={15} /> {saving ? 'Saving…' : 'Save Settings'}</>}
                </button>
            </div>
                ...
                <div className="settings-fields">
                    {FIELDS.map(f => (
                        <div key={f.key} className="settings-field">
                            ...
                            {f.type === 'textarea' ? (
                                <textarea className="settings-input settings-textarea" value={values[f.key] ?? ''} onChange={e => setValues(v => ({ ...v, [f.key]: e.target.value }))} rows={3} />
                            ) : (
                                <div className="settings-input-row">
                                    <input type={f.type} className="settings-input" value={values[f.key] ?? ''} onChange={e => setValues(v => ({ ...v, [f.key]: e.target.value }))} />
                                    {f.type === 'url' && values[f.key] && (
                                        <a href={values[f.key]} target="_blank" rel="noreferrer" className="settings-preview-link" title="Open URL"><ExternalLink size={15} /></a>
                                    )}
                                </div>
                            )}
                            {f.key === 'og_image_url' && values[f.key] && (
                                <div className="settings-image-preview"><img src={values[f.key]} alt="OG preview" /></div>
                            )}
                        </div>
                    ))}
                </div>
            ...
            <div className="settings-section">
                ...
                <div className="og-preview-card">
                    {values.og_image_url && <div className="og-preview-image"><img src={values.og_image_url} alt="OG" /></div>}
